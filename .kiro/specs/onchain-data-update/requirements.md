...
- **パフォーマンス最適化**: キャッシュ機構により API コストとレスポンス時間を削減
...
### Requirement 2: マルチチェーン保有者数データ取得

**Objective:** データアナリストおよび JPYC 保有者として、Ethereum、Polygon、Avalanche の3チェーンにおける JPYC 保有者数の合計を確認したい。これにより、JPYC コミュニティの成長を把握できる。

#### Acceptance Criteria

1. WHEN ダッシュボードが読み込まれる THEN システムは Ethereum、Polygon、Avalanche の各チェーンスキャン API から保有者数データを取得すること
2. WHEN 各チェーンからデータ取得が完了する THEN システムは3チェーン分の保有者数を合算し、合計値を表示すること
3. IF ブラックリストアドレスが設定されている AND ブラックリストアドレスが JPYC を保有している THEN システムはブラックリストアドレスを保有者数から除外すること
4. WHEN 保有者数データが更新される THEN システムは24時間変化量を計算し表示すること
5. IF データ取得エラーが発生する THEN システムはエラーメッセージを表示し、前回のキャッシュデータ（存在する場合）を表示すること

---

### Requirement 3: チェーン別データ可視化

**Objective:** データアナリストとして、各チェーン（Ethereum、Polygon、Avalanche）ごとの総供給量と保有者数の分布を視覚的に把握したい。これにより、どのチェーンが最も活発かを理解できる。

#### Acceptance Criteria

1. WHEN ダッシュボードが読み込まれる THEN システムは各チェーンの総供給量をグラフ形式で表示すること
2. WHEN ダッシュボードが読み込まれる THEN システムは各チェーンの保有者数をグラフ形式で表示すること
3. WHERE グラフ表示エリア THE システムは円グラフまたは棒グラフで各チェーンの比率を表示すること
